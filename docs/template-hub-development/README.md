...
| 03. 판매 준비 | 일부 선반영 | 서버 `saleReadiness` 판정 |
| 04. 운영 액션 | 대기 | 분류 변경, 판매 시작·중지, 링크 |
...
03단계가 "일부 선반영"인 이유: 01단계 목록 응답 계약에 `saleReadiness`가 포함돼
있어, 02단계까지만 구현하면서 임시값을 넣으면 계약이 흔들린다. 따라서 서버의
`evaluateTemplateSaleReadiness()`(로열티 판정 포함)는 이미 완전히 구현했다.
03단계의 남은 산출물은 조건별 단위 테스트와, 단건 mutation이 목록과 같은 판정을
재사용하도록 연결하는 부분이다.
