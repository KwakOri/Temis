...
최종 수정: 2026-08-29
상태: Phase 1~6 구현 완료, Phase 7 개선 및 Phase 8 자동 미리보기 코드·원격 migration 적용 완료 (기존 게시 템플릿 preview 보정 대기)
...
9. [Phase 7 — 운영 테스트 후 개선 계획](./07-post-test-remediation-plan.md)
10. [Phase 8 — 자동 미리보기 이미지 저장 계획](./08-auto-preview-image-plan.md)
...
Phase 6: 저장·발행·카탈로그
    ↓
Phase 7: 운영 테스트 후 권한·입력·피드백 개선
    ↓
Phase 8: 발행 revision 기반 자동 미리보기 이미지 저장
```
...
| 01. Studio Core          | 구현 완료                                    | 공통 셸과 Timetable/Thumbnail Adapter           |
...
| 05. 런타임·PNG           | 구현 완료 (코드·자동 검증·route 렌더 확인)   | 제한형 사용자 편집과 이미지 다운로드            |
| 06. 운영 통합            | 구현 완료 (코드·마이그레이션·정적 검증)      | 저장, 발행, 템플릿 분류와 카탈로그 연결         |
| 07. 운영 테스트 개선     | 계획 완료                                    | 권한 정합성, 입력 의미, cover, 저장 피드백 개선 |
| 08. 자동 미리보기 이미지 | 코드·원격 migration 적용 완료 (기존 게시 템플릿 preview 보정 대기) | 발행 revision 기반 이미지 저장과 cover fallback |
