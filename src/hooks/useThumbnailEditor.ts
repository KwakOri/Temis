...
import { createInitialCardFromConfig } from "@/utils/time-table/data";
import { useEffect, useState } from "react";
import { useThumbnailData } from "./useThumbnailData";
import { useThumbnailPersistence } from "./useThumbnailPersistence";
import { useTimeTableState } from "./useTimeTableState";
import { useTimeTableTheme } from "./useTimeTableTheme";

/**
 * ThumbnailEditor의 모든 상태와 로직을 통합 관리하는 메인 훅
 * CardInputConfig를 받아서 단일 카드를 관리하는 방식
 *
 * 이 훅은 다음과 같은 관심사를 분리하여 관리합니다:
 * - 전역 상태 (useTimeTableState) - 스크린샷, 이미지 편집 등
 * - 데이터 상태 (useThumbnailData) - CardInputConfig 기반 단일 카드
 * - 테마 상태 (useTimeTableTheme)
 * - 지속성 관리 (useThumbnailPersistence)
 */
export const useThumbnailEditor = ({
  cardInputConfig,
  ...
  // 전역 상태 (Context에서 관리) - 스크린샷, 이미지 편집 등
  ...
  const {
    ...
    addEntry,
    removeEntry,
  } = useThumbnailData({ cardInputConfig });
  ...
  useEffect(() => {
    if (!isInitialized) {
      ...
      if (persistedData && persistedData.data) {
        ...
        if (configMatches) {
          // 설정이 일치하면 저장된 데이터 복원
          updateCard(persistedData.data);
          if (persistedData.theme) {
            updateTheme(persistedData.theme);
          }
        } else {
          // 설정이 다르면 새로운 기본값으로 초기화
          const newDefaultCard = createInitialCardFromConfig({
            cardInputConfig,
          });
          updateCard(newDefaultCard);
        }
      }
