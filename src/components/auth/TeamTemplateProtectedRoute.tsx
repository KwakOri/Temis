...
import { useTeamTemplateAccess } from "@/hooks/useTeamTemplateAccess";
import { usePathname, useRouter } from "next/navigation";
import { useEffect } from "react";

interface TeamTemplateProtectedRouteProps {
  children: React.ReactNode;
  templateId: string;
  fallback?: React.ReactNode;
}

export default function TeamTemplateProtectedRoute({
  children,
  templateId,
  fallback,
}: TeamTemplateProtectedRouteProps) {
  const { user, loading: authLoading } = useAuth();
  const {
    hasAccess,
    loading: accessLoading,
    error,
  } = useTeamTemplateAccess({ templateId });
      ...
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-4">
            팀 템플릿 접근 권한을 확인하는 중 오류가 발생했습니다.
          </p>
