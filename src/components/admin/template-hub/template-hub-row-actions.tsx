...
const describeError = (error: unknown): string => {
  if (error instanceof TemplateHubRequestError) {
    if (error.reasons && error.reasons.length > 0) {
      return [error.message, ...error.reasons.map((reason) => `- ${reason.message}`)].join(
        "\n"
      );
    }
...
export const TemplateHubRowActions = ({ item }: { item: TemplateHubItem }) => {
  ...
  const handleSaleToggle = (visible: boolean) => {
    if (!visible && !confirm(`"${item.name}" 템플릿의 판매를 중지하시겠습니까?`)) {
      return;
    }
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5">
      {item.templateEngine === "studio" ? (
        <>
          <Link
            className={cn(actionButton, "bg-blue-50 text-blue-600 hover:bg-blue-100")}
            href={`/admin/template-studio/${item.id}/edit`}
          >
      ...
      {productLinkBlocked ? (
        ...
        <Link
          className={cn(actionButton, "border border-gray-300 text-gray-700 hover:bg-gray-50")}
      ...
      <button
        className={cn(actionButton, "border border-gray-300 text-gray-600 hover:bg-gray-50")}
        type="button"
      ...
      {item.salesType === "custom" ? (
        <button
          className={cn(actionButton, "bg-indigo-50 text-indigo-700 hover:bg-indigo-100")}
          disabled={updateSalesType.isPending}
          type="button"
          onClick={() => handleSalesTypeChange("general")}
        >
          {updateSalesType.isPending && <Loader2 className="h-3.5 w-3.5 animate-spin" />}
          일반 판매로 변경
        </button>
      ) : (
        <button
      ...
      {item.isShopVisible ? (
        ...
        >
          {updateSale.isPending && <Loader2 className="h-3.5 w-3.5 animate-spin" />}
          판매 중지
        </button>
      ) : (
        <button
          className={cn(actionButton, "bg-green-50 text-green-700 hover:bg-green-100")}
