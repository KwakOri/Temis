...
export default function PurchaseManagement() {
  const [purchaseRequests, setPurchaseRequests] = useState<PurchaseRequestWithTemplate[]>([]);
  ...
  const approvePurchaseRequest = async (requestId: string, templateId: string, customerEmail: string) => {
    ...
    try {
      ...
      if (userError || !userData) {
        alert('해당 이메일의 사용자를 찾을 수 없습니다. 사용자가 먼저 회원가입을 해야 합니다.');
        return;
      }
  ...
  const completedRequests = purchaseRequests.filter(req => req.status !== 'pending');
  ...
  return (
    <div className="space-y-6">
                ...
                {pendingRequests.map((request) => (
                  <tr key={request.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                      <button
                        onClick={() => approvePurchaseRequest(request.id, request.template_id!, request.customer_email)}
                        disabled={processingId === request.id}
                        className="bg-green-600 text-white px-3 py-1 rounded text-xs hover:bg-green-700 disabled:opacity-50"
                      >
                        {processingId === request.id ? '처리중...' : '결제확인 및 권한부여'}
                      </button>
                      <button
      ...
      <div className="bg-white shadow rounded-lg">
        <div className="px-6 py-4 border-b border-gray-200">
          <h2 className="text-lg font-medium text-gray-900">처리 완료된 신청</h2>
        </div>
