...
import { CreatePriceOptionInput } from "@/types/priceOption";
import { NextRequest, NextResponse } from "next/server";

// GET: 모든 가격 옵션 조회 (카테고리별 필터링 가능)
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.error("Admin price options fetch error:", error);
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  ...
  try {
    const body: CreatePriceOptionInput = await request.json();
    ...
    if (checkError && checkError.code !== "PGRST116") {
      console.error("Price option value check error:", checkError);
      throw checkError;
    }
    ...
    if (insertError) {
      console.error("Price option creation error:", insertError);
      throw insertError;
    }
    ...
    return NextResponse.json(
      {
        success: true,
        message: "가격 옵션이 성공적으로 생성되었습니다.",
        option: newOption,
      },
      { status: 201 }
    );
  } catch (error) {
    console.error("Price option creation error:", error);
    return NextResponse.json(
      { error: "가격 옵션 생성 중 오류가 발생했습니다." },
      { status: 500 }
    );
  }
}
