...
const TWITTER_CALLBACK_URL = process.env.TWITTER_CALLBACK_URL || `${process.env.NEXT_PUBLIC_BASE_URL}/api/auth/twitter/callback`;
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (action === 'request_token') {
      ...
      const authHeader = generateOAuthHeader('POST', requestTokenUrl, oauthParams, TWITTER_API_KEY, TWITTER_API_SECRET);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const authHeader = generateOAuthHeader('POST', accessTokenUrl, oauthParams, TWITTER_API_KEY, TWITTER_API_SECRET, oauthTokenSecret);
