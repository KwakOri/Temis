...
export const v2_RENDER_CONFIG_STORAGE_PREFIX = 'v2-template-render-config';
...
const TemplateEditorClient = ({
  ...
  const resolvedTemplateId = templateId ?? exampleData.templateId;
  const storageKey = useMemo(
    () => `${v2_RENDER_CONFIG_STORAGE_PREFIX}:${resolvedTemplateId}`,
    [resolvedTemplateId]
  );
  ...
  useEffect(() => {
    v2_logStorageDebug('resolved storage key', {
      storageKey,
      forcedTemplateId: forcedTemplateId ?? null,
      queryTemplateId: queryTemplateId ?? null,
      resolvedTemplateId,
    });
  }, [forcedTemplateId, queryTemplateId, resolvedTemplateId, storageKey]);
  ...
  const providerValue = useMemo(
    ...
    [isLoading, renderConfig, resolvedTemplateId]
  );
