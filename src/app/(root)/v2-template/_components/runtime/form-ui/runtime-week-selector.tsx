...
import RuntimeCardTitle from "./ui/card-title";

interface RuntimeWeekSelectorProps {
  mondayDateStr: string;
  onDateChange: (dateStr: string) => void;
}

const RuntimeWeekSelector: React.FC<RuntimeWeekSelectorProps> = ({
  mondayDateStr,
  onDateChange,
}) => {
  ...
  return (
    <div className="h-12 gap-4 flex justify-between items-center bg-timetable-card-bg shadow-[0_2px_3.4px_rgba(0,0,0,0.08)] border-2 border-timetable-card-border transition-all duration-200 grow-0 rounded-2xl px-3">
      {/* 날짜 선택 영역 */}
      <RuntimeCardTitle size="sm" label="주간 선택" />
...
export default RuntimeWeekSelector;
