...
import { timeTableStorage, createAutoSave } from './localStorage';
...
export const useFormPersistence = () => {
  ...
  const saveScale = useCallback((scale: number) => {
    return timeTableStorage.saveScale(scale);
  }, []);
   ...
   * 개별 카드 업데이트
   */
  const updateCard = useCallback((dayIndex: number, cardData: Partial<TDefaultCard>, currentData: TDefaultCard[]) => {
  ...
  const clearAllData = useCallback(() => {
    return timeTableStorage.clearAllTimeTableStorage();
  }, []);
...
export const fieldSavers = {
   ...
   * 오프라인 토글 저장
   */
  saveOfflineToggle: (dayIndex: number, isOffline: boolean, currentData: TDefaultCard[]) => {
    return fieldSavers.saveCardField(dayIndex, 'isOffline', isOffline, currentData);
  },

  /**
   * 시간 필드 저장
   */
  saveTimeField: (dayIndex: number, time: string, currentData: TDefaultCard[]) => {
   ...
   * 주제 필드 저장
   */
  saveTopicField: (dayIndex: number, topic: string, currentData: TDefaultCard[]) => {
   ...
   * 설명 필드 저장
   */
  saveDescriptionField: (dayIndex: number, description: string, currentData: TDefaultCard[]) => {
    return fieldSavers.saveCardField(dayIndex, 'description', description, currentData);
  }
};
...
export const backupSystem = {
  ...
  createBackup: (data: TDefaultCard[], theme: TTheme, scale: number) => {
    ...
    return localStorage.setItem('template-timetable-backup', JSON.stringify(backup));
  },
