...
import ThumbnailInputCard from "@/components/TimeTable/FixedComponents/ThumbnailInputCard";
...
const TimeTableEditorContent: React.FC = () => {
  // 통합 상태 관리 훅 사용 - 썸네일 전용 훅으로 변경

  const { state, data, updateCard, currentTheme, resetData, isInitialized } =
    useThumbnailEditor({
  ...
  // 초기화되지 않았으면 로딩 표시 (썸네일은 weekDates가 필요 없음)
...
const TimeTableEditor: React.FC = () => {
  ...
  const { state, actions } = useThumbnailEditor({
    cardInputConfig: CARD_INPUT_CONFIG,
    defaultTheme: defaultTheme,
    captureSize: templateSize,
  });
